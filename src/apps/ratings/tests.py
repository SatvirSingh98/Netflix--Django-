...
class TestRatingModel(TestCase):
    ...
    def test_rating_aggregate(self):
        ...
        passed_avg = total_sum / len(self.rating_totals)
        self.assertEqual(passed_avg, db_avg)

    def test_playlist_rating_aggregate(self):
        item1 = Playlist.objects.aggregate(average=Avg('ratings__value'))['average']
        self.assertIsNotNone(item1)
        self.assertTrue(item1 > 0)
